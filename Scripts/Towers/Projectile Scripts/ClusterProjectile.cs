...
namespace Towers
{
    ...
    {
        [BoxGroup("Projectile Settings"), SerializeField]
        private GameObject
        ...
        [BoxGroup("Projectile Settings"), SerializeField] private float explosionRadius = 0.2f;
        [BoxGroup("Projectile Settings"), SerializeField] private float maxOffset = 0.5f;
        [BoxGroup("Projectile Settings"), SerializeField] private float speedOffset;
        ...
        {   
            ...
            if (IsOverWater())
            {
                impactEffectInstance = Instantiate(waterExplosionPrefab, transform.position, Quaternion.identity, parent: this.transform);
            }
            else
            {
                impactEffectInstance = Instantiate(surfaceExplosionPrefab, transform.position, Quaternion.identity, parent: this.transform);
            }
        ...
        {
            audioManager.PlayOneShot(fmodEvents.catapultProjectileImpactSound, transform.position);
        }
    }
}
